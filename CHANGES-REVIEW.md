...
- Scrolling in the first-run wizard's Backend step (Low): the mouse wheel did nothing while the pointer was over the experimental-driver notice. The notice sits in its own small scroll box inside the scrolling page, and WPF's ScrollViewer takes every wheel event even when it has nothing to scroll. That box, and the same one in the Native PS5 setup sheet, now hands the wheel to the page when it cannot scroll further that way. Tested (the edge decision); confirmed by the owner.
- Native PS5 mode took about 25 s to bring the controller back (Medium): turning it on plugs in a virtual DualSense, and the service restart that follows unplugs it straight away. For 15 s after any VIIPER Sony output is plugged in, controller discovery skipped every pad with a VIIPER Sony product ID, so that the arriving output is not taken for an input; a physical DualSense has the same ID, so the restart found no controller and the pad came back only on a later 10-second recovery scan (both of the owner's logs, admin and not). Pads that were already connected before the output was plugged in are now let through. Tested; the owner measured 1–2 s after the fix.
- Service start pause (Low, C034, decision #8): every service start, including each restart (Native PS5 mode, Hide DS4 Controller), paused 2 s for nothing. The pause came from DS4Windows, where it followed the ViGEmBus connection that Thrum no longer has. It is removed; a start now waits, for up to 2 s, only for a VIIPER backend Thrum launched in the last 10 s, which the virtual outputs it plugs in need. Tested (the wait decision).
- Slow Native PS5 turn-on (diagnostics only, no behaviour change): the owner's log showed 22.7 s between "Using Exclusive Mode" and "Found Controller" in a session running as administrator. With Verbose logging on, the log now marks each step of opening a controller: the exclusive open, whether it was refused and which known controller programs were running, the device disable and enable an administrator session does to take the controller back, and the fallback to shared mode.
